---
title: Multiple control planes with limited exposure
description: Install an Istio mesh across multiple Kubernetes clusters with individually deployed control planes and limited exposure of services.
weight: 2
aliases:
...
1. Generate a multicluster-gateways Istio configuration file using `helm`:
    ...
    $ helm template install/kubernetes/helm/istio --name istio --namespace istio-system \
        ...
        --set gateway.istio-egressgatewat.env.ISTIO_META_REQUESTED_NETWORK_VIEW="" > $HOME/istio.yaml
    {{< /text >}}
...
1. Run the following commands in **every cluster** to deploy an identical Istio control plane
    ...
    *   Create a destination rule to ensure all traffic to *.global is using mTLS:
...
1. Create a service entry for the `httpbin` service in `cluster2`.
    ...
    1.  Create a destination rule to set SNI to `httpbin.bar.global`
        ...
        kind: DestinationRule
        metadata:
          name: httbin-bar-global
        spec:
          host: httpbin.bar.global
          trafficPolicy:
            tls:
              mode: ISTIO_MUTUAL
              sni: httpbin.bar.global
        EOF
        {{< /text >}}

    1.  [Create a passthrough ingress gateway](/docs/tasks/traffic-management/ingress/ingress-sni-passthrough/)
        for `httpbin`:
    ...
    1.  Configure routing for `httpbin`:
